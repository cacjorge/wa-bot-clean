...
exports.run = async (bot, message, args) => {
	if (message.isMedia && message.type === 'image' ) {
		try {
			...
			await bot.sendImage(message.from, `data:${message.mimetype};base64,${result.base64img}`,{ keepScale: true });
		} catch(err) {
			console.log(err);
		}
		...
		try {
			...
			var result = await removeBackgroundFromImageBase64({ base64img, apiKey: 'jStop5xAeABgDDC5Lx6Nv9EZ', size: 'auto', type: 'auto', outFile });
			//await fs.writeFile(outFile, result.base64img);
			await bot.sendImage(message.from, `data:${message.quotedMsg.mimetype};base64,${result.base64img}`,{ keepScale: true });
		} catch(err) {
			console.log(err);
		}
	} else {
...
exports.help = {
    name: "No Background",
    description: "Removing Background from a picture",
    usage: "nobg <image>/<quote image>/<url>",
    cooldown: 5
};